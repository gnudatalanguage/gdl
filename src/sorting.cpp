...
#define MERGESORT_PARALLEL_THRESHOLD 1E5
...
#define CREATE_HISTOGRAMS2(type, buffer)													\
	...
	ZeroMemory(Histogram, 256*2*sizeof(T));                                                 \
																							\
	/* Prepare to count */																	\
...
#define CREATE_HISTOGRAMS4(type, buffer)													\
	/* Clear counters/histograms */															\
	ZeroMemory(Histogram, 256*4*sizeof(T));                                             	\
																							\
	/* Prepare to count */																	\
	const DByte* p = (const DByte*)input;													\
...
#define CREATE_HISTOGRAMS8(type, buffer)													\
	/* Clear counters/histograms */															\
	ZeroMemory(Histogram, 256*8*sizeof(T));                                                 \
																							\
	/* Prepare to count */																	\
	const DByte* p = (const DByte*)input;													\
...
static T* RadixSort(const DByte* input, SizeT nb)
{
    ...
    /* Clear counters/histograms */
    ZeroMemory(Histogram, 256 * sizeof (T));
...
inline void insertionSort (T* val, IndexT* index, SizeT lo, SizeT hi) 
{
...
template<typename T, typename IndexT>
inline void insertionSortAux (IndexT* aux, IndexT* index,  SizeT lo, SizeT hi, T* val) 
{
    for (SizeT i = lo; i <= hi; ++i)
    {
        for (SizeT j = i; j > lo && less(val[index[j]], val[index[j-1]]); --j)
    ...
    memcpy(&(aux[lo]), &(index[lo]), (hi-lo) * sizeof (IndexT)); //give back sub
}
...
template <typename T, typename IndexT>
void QuickSortIndex(T* val, IndexT* index, DLong left, DLong right) 
{ 
    ...
    {
        insertionSort(val, index, left, right);
        return;
    }
...
//    QuickSortIndex(val, index, lo, iterJ);
//    QuickSortIndex(val, index, iterI, hi);
...
#pragma omp parallel for num_threads(2) if (length >= MERGESORT_PARALLEL_THRESHOLD && CpuTPOOL_NTHREADS > 1)
  ...
  static void MergeSortIndexAux(IndexT* aux, IndexT* index, SizeT low, SizeT high, T* val)
  {
    SizeT length = high - low + 1;
      if (length < 2) return;
    if (length < INSERTION_SORT_THRESHOLD) {
      insertionSortAux(aux, index, low, high, val);
      return;
    } 
...
#pragma omp parallel for num_threads(2) if (length >= MERGESORT_PARALLEL_THRESHOLD && CpuTPOOL_NTHREADS > 1)
  ...
  static void AdaptiveSortIndexAux(IndexT* aux, IndexT* index, SizeT low, SizeT high, T* val)
  {
    ...
    if (length < INSERTION_SORT_THRESHOLD) {
      ...
      IndexT* res=RadixSort<IndexT>(val, length);
      memcpy(&(index[low]), res, length*sizeof(IndexT));
      gdlAlignedFree(res);
      return;
    }
    
//    SizeT mid = low + (high - low) / 2;
//    SortIndexAux(index, aux, low, mid, val);
//    SortIndexAux(index, aux, mid+1, high, val);
...
// for checking if sorted array is indeed sorted.  
  template< typename T, typename IndexT>
  inline bool sortcheck(T* val, IndexT* index, SizeT nEl)
  {
    // all indexes [0 to n-1] should be present and result should be in increasing order.
    //check on "total" being n*(n-1)/2 is not optimal but should do.
    SizeT total=0;
#pragma omp parallel if (nEl >= CpuTPOOL_MIN_ELTS && (CpuTPOOL_MAX_ELTS == 0 || CpuTPOOL_MAX_ELTS <= nEl)) shared(total)
    {
#pragma omp for reduction(+:total)
    for (SizeT j = 0; j < nEl; ++j) total+=index[j];
    }
    SizeT totalref=nEl*(nEl-1)/2;
    if (total != totalref) { std::cerr<<totalref<<"!="<<total<<std::endl; return false;}    
    SizeT i = 0;
    while (!finite(val[index[i]])) {
      i++;
      if (i >= nEl) return true;
    }
    T curval = val[index[i++]];
    for (; i < nEl;) {
      if (val[index[i]] < curval) return false;
      curval = val[index[i++]];
    }
    return true;
  }  
//--------------------------------------------------------------------------------------------------------------------
// Sorting algos: The "private" GDL_SORT enables keywords QUICK,MERGE,RADIX,INSERT,CHECK. Those are not there to for the user
...
// Default values chosen are for my intel i7, 4 cores, 8 threads.
  ...
  BaseGDL* gdl_sort_fun( EnvT* e)
  {
    ...
    if (!(radix || quick ||merge|| insert)) e->Throw("I need one of QUICK, MERGE or RADIX keyword set.");

    static int checkIx = e->KeywordIx("CHECK");
    bool check = e->KeywordSet(checkIx);
    ...
    if (radix) {
        ...
        if (check && !sortcheck(val,index,nEl)) e->Throw("check failed.");
	...
	DUInt* val = (DUInt*)(static_cast<DUIntGDL*> (p0)->DataAddr());
	  ...
	  DByte* val = (DByte*)(static_cast<DByteGDL*> (p0)->DataAddr());
        ...
        if (check && !sortcheck(magnitude,index,nEl)) e->Throw("check failed.");
        delete[] magnitude;
      ...
      if (p0->Type() == GDL_DOUBLE) {
        ...
        if (merge) {
          ...
          insertionSort( val, hh, low, high);
        } else {
          assert(false);
        }
        if (check && !sortcheck(val,hh,nEl)) e->Throw("check failed.");
        return res;
    } else if (p0->Type() == GDL_FLOAT) {
        DFloat* val = (DFloat*)(static_cast<DFloatGDL*>(p0)->DataAddr());
        DLongGDL* res = new DLongGDL(dimension(nEl), BaseGDL::INDGEN);
      ...
      if (check && !sortcheck<DInt, DLong>( val, hh, nEl)) e->Throw("check failed.");
      return res;
    } else if (p0->Type() == GDL_UINT) {
      DUInt* val = (DUInt*)(static_cast<DUIntGDL*>(p0)->DataAddr());
      DLongGDL* res = new DLongGDL(dimension(nEl), BaseGDL::INDGEN);
      ...
      if (merge) {
        ...
        QuickSortIndex( magnitude, hh, low, high);
          ...
          insertionSort( magnitude, hh, low, high);
      } else {
        assert(false);
      }
      if (check && !sortcheck(magnitude,hh,nEl)) e->Throw("check failed.");
      delete[] magnitude;
      return res;
    } else if (p0->Type() == GDL_COMPLEXDBL) {
      DComplexDblGDL* p0F = static_cast<DComplexDblGDL*> (p0);
