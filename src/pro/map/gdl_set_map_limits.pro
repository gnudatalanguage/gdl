...
;    longitudes: beautify
     ...
     if (lonmin eq lonmax) then  begin lonmin=p0lon-180 & if lonmin lt -180 then lonmin+=360 & lonmax=lonmin+360 & end
...
;    latitudes: beautify
	 ...
	 danger=90
	 if strpos(myMap.up_name,"=igh ") then danger=85 ; even 89 does not pass
     ...
     if ( map_limits[1] eq  map_limits[3])  then message,/informational,"GDL_SET_MAP_LIMITS: Warning, MAP limits are invalid."
