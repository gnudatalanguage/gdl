...
;
cflag = KEYWORD_SET(center)	; 0=start, 1=center.
...
IF (N_ELEMENTS(mx) NE 0) THEN BEGIN
    fx= (minone gt 0)? float(dims[0]-1)/(mx-1) : float(dims[0])/mx
    x = (cflag gt 0) ?fx*(FINDGEN(mx)+0.5)-0.5 : fx*FINDGEN(mx)
    nopt=1
ENDIF
;
...
IF (N_ELEMENTS(my) NE 0 && ndim ge 2) THEN BEGIN
    fy= (minone gt 0)? float(dims[1]-1)/(my-1) : float(dims[1])/my
    y = (cflag gt 0) ?fy*(FINDGEN(my)+0.5)-0.5 : fy*FINDGEN(my)
    nopt=2
ENDIF
;
...
IF (N_ELEMENTS(mz) NE 0 && ndim ge 3) THEN BEGIN
    fz= (minone gt 0) ? float(dims[2]-1)/(mz-1) : float(dims[2])/mz
    z = (cflag gt 0) ? fz*(FINDGEN(mz)+0.5)-0.5 : fz*FINDGEN(mz)
    nopt=3
ENDIF
;
