...
;
; PURPOSE: 
;       Scales an input image by equalising its histogram. By setting a percent
;       it can an also scale an image via a linear contrast.
;
;
; CATEGORY:
;        Image processing.
;
; CALLING SEQUENCE:
;        Result = hist_equal(image, BinSIZE=value, Fcn=column_vector, Histogram_Only=value, Maxv=value, Minv=value, Omax=variable, Omin=variable, Percent=value, Top=value) 
;
;         
;
; KEYWORD PARAMETERS: 
;         BINSIZE:  The SIZE of the bin in which to construct the histogram. Defaulted to 1 for an image of type BYTE. The default for other data types is (maxv-minv)/5000double.
;
;         FCN: The cumulative probability distribution function. (Not currently implemented).
;
;         HISTOGRAM_ONLY: Returns the cumulative histogram of type Long.
;
;         MAXV: The maximum value of the original image to consider. Defaulted to 255 if image is of type BYTE, otherwise the image is searched for the maximum value.
;
;        MINV: The minimum value of the original image to consider. Default is 0 if image is of type BYTE, otherwise the image is searched for the minimum value.
;
;       PERCENT: If set to a value between 0 and 100, the histogram of the image will be linearly stretched between the upper and lower values found by the specified percent of the cumulative histogram.
;
;       TOP: The maximum value of the scaled result. Defaulted to 255.
;
; OUTPUTS:
;       An image that is has been modified by its histogram. If the Histogram_Only keyword has been set, then the cumulative histogram (of type Long) will be output.
;
;
; OPTIONAL OUTPUTS:
;
...
;       22-May-2012    Alain Coulais Details (/help,/test), initial
;       import in GDL
;
;-
; LICENCE:
