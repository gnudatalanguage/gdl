...
;
; CALLING SEQUENCE: 
;      result = DERIV, [x], y, [help=help], [test=test], [no_check=no_check]
;
...
;                   - We do check if the x(n+1)-x(n) is <> 0. (not
;                     done in IDL)
;                   - We do check if the X array is well sorted (not
;                     done in IDL)
;                   - We do check if Delta_x is constant or not (not
;                     done in IDL)
...
;
; WARNING: 
; - because current behavior (GDL 0.9pre) of MESSAGE and .continue ans .skip
; differs from the IDL ones, MESSAGE is substituted by "print, '%'"
; - NaN values are not managed now
;
; PROCEDURE:  
...
; 
;-
function DERIV, x, y, help=help, test=test, no_check=no_check
;
ON_ERROR, 2
;
...
if KEYWORD_SET(help) then begin
    print, 'function DERIV, [x,] y, [help=help, test=test, no_check=no_check]'
    return, -1
endif
;
nbp_x=N_ELEMENTS(x)
