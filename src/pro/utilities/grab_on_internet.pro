...
; ---------------------------------------------------
;
pro GRAB_ON_INTERNET, list_of_url, quiet=quiet, overwrite=overwrite, status=status, $
                      verbose=verbose, test=test, help=help
;
ON_ERROR, 2
;
if KEYWORD_SET(help) then begin
   print, 'pro GRAB_ON_INTERNET, list_of_url, overwrite=overwrite, status=status, $'
...
if KEYWORD_SET(overwrite) then begin
   ...
   for ii=0, N_ELEMENTS(list_of_url)-1 do begin
      ...
      if ~FILE_TEST(cur_file) then begin 
         SPAWN, cmd+list_of_url[ii], exit_status=exit_status
         if (exit_status NE 0) then print, 'Problem with URL : ', list_of_url[ii]
      endif else begin
