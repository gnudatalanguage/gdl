...
#pragma omp for
  ...
  {
    e->NParam( 1);//, "FLOOR");
	DType  pType =e->GetPar(0)->Type();     
    if (!(NumericType(pType))) e->Throw(e->GetPar(0)->TypeStr()+" expression: not allowed in this context: "+e->GetParString(0));
