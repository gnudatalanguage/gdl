/* src.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int sph_addnod_(integer *nst, integer *k, doublereal *x, 
	...
	*lend, integer *lnew, integer *ier)
{
    ...
    /* Local variables */
    integer l;
    doublereal p[3], b1, b2, b3;
    integer i1, i2, i3, kk, lp, in1, io1, io2, km1, lpf, ist, lpo1;
    extern /* Subroutine */ int sph_swap_(integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *);
    integer lpo1s;
    extern /* Subroutine */ int sph_bdyadd_(integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *), sph_intadd_(integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *, 
	    integer *), sph_trfind_(integer *, doublereal *, integer *, 
	    doublereal *, doublereal *, doublereal *, integer *, integer *, 
	    integer *, doublereal *, doublereal *, doublereal *, integer *, 
	    integer *, integer *), covsph_(integer *, integer *, integer *, 
	    integer *, integer *, integer *);
    extern integer sph_lstptr_(integer *, integer *, integer *, integer *);
    extern logical sph_swptst_(integer *, integer *, integer *, integer *, 
...
/*   from node K. */
    ...
    sph_trfind_(&ist, p, &km1, &x[1], &y[1], &z__[1], &list[1], &lptr[1], &lend[1]
	    , &b1, &b2, &b3, &i1, &i2, &i3);
...
/*   Test for collinear or duplicate nodes. */
	...
	sph_intadd_(&kk, &i1, &i2, &i3, &list[1], &lptr[1], &lend[1], lnew);
    } else {
	if (i1 != i2) {
	    sph_bdyadd_(&kk, &i1, &i2, &list[1], &lptr[1], &lend[1], lnew);
	} else {
...
L1:
    lp = sph_lstptr_(&lend[io1], &io2, &list[1], &lptr[1]);
    if (list[lp] < 0) {
	goto L2;
    }
    lp = lptr[lp];
...
/*             opposite K and must be tested. */
    ...
    lpo1s = lpo1;
    if (! sph_swptst_(&in1, &kk, &io1, &io2, &x[1], &y[1], &z__[1])) {
	goto L2;
    }
    sph_swap_(&in1, &kk, &io1, &io2, &list[1], &lptr[1], &lend[1], &lpo1);
    if (lpo1 == 0) {
...
doublereal areas_(doublereal *v1, doublereal *v2, doublereal *v3)
{
    ...
    /* Local variables */
    integer i__;
    doublereal a1, a2, a3, s12, s31, s23, u12[3], u23[3], u31[3], ca1, ca2, 
	    ca3, dv1[3], dv2[3], dv3[3];
...
/* Subroutine */ int sph_bdyadd_(integer *kk, integer *i1, integer *i2, integer *
	list, integer *lptr, integer *lend, integer *lnew)
{
    integer k, n1, n2, lp, lsav, nsav, next;
    extern /* Subroutine */ int sph_insert_(integer *, integer *, integer *, 
	    integer *, integer *);
...
L1:
    lp = lend[next];
    sph_insert_(&k, &lp, &list[1], &lptr[1], lnew);
    if (next == n2) {
	goto L2;
    }
    next = -list[lp];
...
/* Subroutine */ int sph_bnodes_(integer *n, integer *list, integer *lptr, 
...
/* Subroutine */ int sph_circum_(doublereal *v1, doublereal *v2, doublereal *v3, 
	doublereal *c__, integer *ier)
{
    ...
    /* Local variables */
    integer i__;
    doublereal e1[3], e2[3], cu[3], cnorm;
	...
	integer *ier)
{
    ...
    /* Local variables */
    doublereal c__[3], t;
    integer i1, i2, i3, i4, n0, n1, n2, n3, n4;
    doublereal v1[3], v2[3], v3[3];
    integer lp, kt, nn, nt, nm2, kt1, kt2, kt11, kt12, kt21, kt22, lpl, lpn;
    logical swp;
    integer ierr;
    extern /* Subroutine */ int sph_circum_(doublereal *, doublereal *, 
...
/*     ordering of the vertices. */
	    ...
	    if (! sph_swptst_(&n1, &n2, &n3, &n4, &x[1], &y[1], &z__[1])) {
		goto L5;
	    }
...
/*     of N2, and N1 as a neighbor of N3. */
	...
	lpn = sph_lstptr_(&lpl, &n2, &list[1], &lptr[1]);
	listc[lpn] = kt;
	lpn = sph_lstptr_(&lend[n2], &n3, &list[1], &lptr[1]);
	listc[lpn] = kt;
	lpn = sph_lstptr_(&lend[n3], &n1, &list[1], &lptr[1]);
	listc[lpn] = kt;
L11:
	if (lp != lpl) {
	    goto L10;
...
/* Subroutine */ int sph_delarc_(integer *n, integer *io1, integer *io2, integer *
	list, integer *lptr, integer *lend, integer *lnew, integer *ier)
{
    ...
    extern /* Subroutine */ int sph_delnb_(integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *);
    extern integer sph_lstptr_(integer *, integer *, integer *, integer *);
...
/*   no longer be valid following the call to DELNB. */
    ...
    sph_delnb_(&n1, &n2, n, &list[1], &lptr[1], &lend[1], lnew, &lph);
    if (lph < 0) {
	*ier = 4;
	return 0;
    }
...
/*   neighbor. */
    ...
    sph_delnb_(&n2, &n1, n, &list[1], &lptr[1], &lend[1], lnew, &lph);

/* Make N3 a boundary node with first neighbor N2 and last */
/*   neighbor N1. */

    lp = sph_lstptr_(&lend[n3], &n1, &list[1], &lptr[1]);
    lend[n3] = lp;
    list[lp] = -n1;
...
/* Subroutine */ int sph_delnb_(integer *n0, integer *nb, integer *n, integer *
...
/* Subroutine */ int sph_delnod_(integer *k, integer *n, doublereal *x, 
	doublereal *y, doublereal *z__, integer *list, integer *lptr, integer 
	*lend, integer *lnew, integer *lwk, integer *iwk, integer *ier)
{
    ...
    /* Local variables */
    integer i__, j, n1, n2;
    doublereal x1, x2, y1, y2, z1, z2;
    integer nl, lp, nn, nr;
    doublereal xl, yl, zl, xr, yr, zr;
    integer nnb, lp21, lpf, lph, lpl, lpn, iwl, nit, lnw, lpl2;
    extern logical sph_left_(doublereal *, doublereal *, doublereal *, doublereal 
	    *, doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *);
    logical bdry;
    integer ierr, lwkl;
    extern /* Subroutine */ int sph_swap_(integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *), sph_delnb_(
	    integer *, integer *, integer *, integer *, integer *, integer *, 
	    integer *, integer *);
    extern integer sph_nbcnt_(integer *, integer *);
    extern /* Subroutine */ int sph_optim_(doublereal *, doublereal *, doublereal 
	    *, integer *, integer *, integer *, integer *, integer *, integer 
	    *, integer *);
    integer nfrst;
    extern integer sph_lstptr_(integer *, integer *, integer *, integer *);
...
/*     a boundary node, then N2 LEFT NL->NR. */
    ...
    lpl2 = lend[n2];
    if (! ((bdry || sph_left_(&xr, &yr, &zr, &xl, &yl, &zl, &x1, &y1, &z1)) && (
	    list[lpl2] < 0 || sph_left_(&xl, &yl, &zl, &xr, &yr, &zr, &x2, &y2, &
	    z2)))) {
...
/*     since N1 will be deleted from X, Y, and Z. */
    ...
    sph_swap_(&nl, &nr, &n1, &n2, &list[1], &lptr[1], &lend[1], &lp21);
    if (lp21 == 0) {
	nr = n2;
	xr = x2;
	yr = y2;
...
/*   Recompute the LIST indexes and NFRST, and decrement NNB. */
    ...
    lpf = lptr[lpl];
    nfrst = list[lpf];
    lp = sph_lstptr_(&lpl, &nl, &list[1], &lptr[1]);
    if (nr == nfrst) {
	goto L2;
    }
...
L3:
	...
	if (nnb > 3) {
	    ...
	    n2 = list[lp];
	    nl = list[lpl];
	    bdry = sph_left_(&x[nr], &y[nr], &z__[nr], &x[nl], &y[nl], &z__[nl], &
		    x[n2], &y[n2], &z__[n2]);
	}
	if (bdry) {
...
L6:
    ...
    sph_delnb_(&n2, &n1, n, &list[1], &lptr[1], &lend[1], &lnw, &lph);
    if (lph < 0) {
	goto L23;
    }
...
/*   in IWK. */
    ...
    *lnew = lnw;
    if (iwl > 0) {
	nit = iwl << 2;
	sph_optim_(&x[1], &y[1], &z__[1], &iwl, &list[1], &lptr[1], &lend[1], &
		nit, &iwk[3], &ierr);
	if (ierr != 0 && ierr != 1) {
	    goto L25;
	}
...
/* Subroutine */ int sph_edge_(integer *in1, integer *in2, doublereal *x, 
	...
	list, integer *lptr, integer *lend, integer *ier)
{
    ...
    doublereal x0, x1, x2, y0, y1, y2, z0, z1, z2;
    integer nl, lp, nr;
    doublereal dp12;
    integer lp21, iwc, iwf, lft, lpl, iwl, nit;
    doublereal dp1l, dp2l, dp1r, dp2r;
    extern logical sph_left_(doublereal *, doublereal *, doublereal *, doublereal 
	    *, doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *);
    integer ierr;
...
L3:
    if (sph_left_(&x2, &y2, &z2, &x1, &y1, &z1, &x[nl], &y[nl], &z__[nl])) {
	goto L4;
    }
    lp = lptr[lp];
    nl = list[lp];
...
L4:
    ...
    lp = lptr[lp];
    nl = (i__1 = list[lp], abs(i__1));
    if (sph_left_(&x1, &y1, &z1, &x2, &y2, &z2, &x[nl], &y[nl], &z__[nl])) {
...
/*   Set NL or NR to NEXT. */
    ...
    if (sph_left_(&x1, &y1, &z1, &x2, &y2, &z2, &x[next], &y[next], &z__[next])) {
	nl = next;
    } else {
	nr = next;
    }
...
/*     swap. */
    ...
    if (! sph_left_(&x0, &y0, &z0, &x[nr], &y[nr], &z__[nr], &x[next], &y[next], &
	    z__[next])) {
	goto L14;
    }
    if (lft >= 0) {
    ...
    if (! sph_left_(&x[nl], &y[nl], &z__[nl], &x0, &y0, &z0, &x[next], &y[next], &
	    z__[next])) {
	goto L14;
    }
...
/*   N0 RIGHT N1->N2.  Test for a possible swap. */
    ...
    if (! sph_left_(&x0, &y0, &z0, &x[nr], &y[nr], &z__[nr], &x2, &y2, &z2)) {
	goto L10;
    }
...
L22:
    if (! sph_left_(&x[nl], &y[nl], &z__[nl], &x0, &y0, &z0, &x2, &y2, &z2)) {
	goto L10;
    }
...
L24:
    sph_swap_(&n2, &n1, &nl, &nr, &list[1], &lptr[1], &lend[1], &lp21);
    iwk[(iwc << 1) + 1] = 0;
    iwk[(iwc << 1) + 2] = 0;
...
/* Subroutine */ int sph_getnp_(doublereal *x, doublereal *y, doublereal *z__, 
...
/* Subroutine */ int sph_insert_(integer *k, integer *lp, integer *list, integer *
	lptr, integer *lnew)
{
    integer lsav;
	...
	doublereal *zv, integer *nv, integer *listv, integer *ier)
{
    ...
    doublereal q[3];
    integer i1, i2, k0;
    doublereal v1[3], v2[3], cn[3], bp, bq;
    integer ni;
    doublereal pn[3], qn[3], vn[3];
    integer imx;
    logical lft1, lft2, even;
    integer ierr;
    logical pinr, qinr;
    doublereal qnrm, vnrm;
...
/* Subroutine */ int sph_intadd_(integer *kk, integer *i1, integer *i2, integer *
...
/* Add K as a neighbor of I1, I2, and I3. */
    ...
    lp = sph_lstptr_(&lend[n1], &n2, &list[1], &lptr[1]);
    sph_insert_(&k, &lp, &list[1], &lptr[1], lnew);
    lp = sph_lstptr_(&lend[n2], &n3, &list[1], &lptr[1]);
    sph_insert_(&k, &lp, &list[1], &lptr[1], lnew);
    lp = sph_lstptr_(&lend[n3], &n1, &list[1], &lptr[1]);
    sph_insert_(&k, &lp, &list[1], &lptr[1], lnew);
...
integer sph_jrand_(integer *n, integer *ix, integer *iy, integer *iz)
{
    /* System generated locals */
    integer ret_val;
...
logical sph_left_(doublereal *x1, doublereal *y1, doublereal *z1, doublereal *x2, 
...
integer sph_lstptr_(integer *lpl, integer *nb, integer *list, integer *lptr)
{
    /* System generated locals */
    integer ret_val;
...
integer sph_nbcnt_(integer *lpl, integer *lptr)
{
    /* System generated locals */
    integer ret_val;
...
integer sph_nearnd_(doublereal *p, integer *ist, integer *n, doublereal *x, 
	doublereal *y, doublereal *z__, integer *list, integer *lptr, integer 
	*lend, doublereal *al)
{
    /* System generated locals */
    ...
    integer l;
    doublereal b1, b2, b3;
    integer i1, i2, i3, n1, n2, n3, lp, nn, nr;
    doublereal ds1;
    integer lp1, lp2;
    doublereal dx1, dx2, dx3, dy1, dy2, dy3, dz1, dz2, dz3;
    integer lpl;
    doublereal dsr;
    integer nst, listp[25], lptrp[25];
    extern /* Subroutine */ int sph_trfind_(integer *, doublereal *, integer *, 
	    doublereal *, doublereal *, doublereal *, integer *, integer *, 
	    integer *, doublereal *, doublereal *, doublereal *, integer *, 
	    integer *, integer *);
    extern integer sph_lstptr_(integer *, integer *, integer *, integer *);
...
/*   from P. */
    ...
    sph_trfind_(&nst, &p[1], n, &x[1], &y[1], &z__[1], &list[1], &lptr[1], &lend[
	    1], &b1, &b2, &b3, &i1, &i2, &i3);
...
/* Subroutine */ int sph_optim_(doublereal *x, doublereal *y, doublereal *z__, 
...
/*   Test IO1-IO2 for a swap, and update IWK if necessary. */
	...
	if (! sph_swptst_(&n1, &n2, &io1, &io2, &x[1], &y[1], &z__[1])) {
	    goto L4;
	}
	sph_swap_(&n1, &n2, &io1, &io2, &list[1], &lptr[1], &lend[1], &lp21);
	if (lp21 == 0) {
	    goto L9;
	}
	swp = TRUE_;
...
doublereal sph_store_(doublereal *x)
{
    /* System generated locals */
    doublereal ret_val;
...
/* Subroutine */ int sph_swap_(integer *in1, integer *in2, integer *io1, integer *
...
/* Test for IN1 and IN2 adjacent. */
    ...
    /* Function Body */
    lp = sph_lstptr_(&lend[*in1], in2, &list[1], &lptr[1]);
...
/* Delete IO2 as a neighbor of IO1. */
    ...
    lp = sph_lstptr_(&lend[*io1], in2, &list[1], &lptr[1]);
    lph = lptr[lp];
    lptr[lp] = lptr[lph];
...
/*   using the hole created above. */
    ...
    lp = sph_lstptr_(&lend[*in1], io1, &list[1], &lptr[1]);
    lpsav = lptr[lp];
    lptr[lp] = lph;
    list[lph] = *in2;
    lptr[lph] = lpsav;
...
/* Delete IO1 as a neighbor of IO2. */
    ...
    lp = sph_lstptr_(&lend[*io2], in1, &list[1], &lptr[1]);
    lph = lptr[lp];
    lptr[lp] = lptr[lph];
...
/* Insert IN1 as a neighbor of IN2 following IO2. */
    ...
    lp = sph_lstptr_(&lend[*in2], io2, &list[1], &lptr[1]);
    lpsav = lptr[lp];
    lptr[lp] = lph;
    list[lph] = *in1;
    lptr[lph] = lpsav;
...
logical sph_swptst_(integer *n1, integer *n2, integer *n3, integer *n4, 
	doublereal *x, doublereal *y, doublereal *z__)
{
    ...
    /* Local variables */
    doublereal x4, y4, z4, dx1, dx2, dx3, dy1, dy2, dy3, dz1, dz2, dz3;
...
/* Subroutine */ int sph_trfind_(integer *nst, doublereal *p, integer *n, 
	...
	integer *i1, integer *i2, integer *i3)
{
    ...
    doublereal xp, yp, zp;
    integer n1s, n2s;
    doublereal eps, tol, ptn1, ptn2;
    integer next;
    extern integer sph_jrand_(integer *, integer *, integer *, integer *);
    extern doublereal sph_store_(doublereal *);
    extern integer sph_lstptr_(integer *, integer *, integer *, integer *);
...
/*     node opposite N2->N1) unless N1->N2 is a boundary arc. */
	...
	lp = sph_lstptr_(&lend[n2], &n1, &list[1], &lptr[1]);
	if (list[lp] < 0) {
	    goto L9;
	}
	lp = lptr[lp];
...
/* Subroutine */ int sph_trlist_(integer *n, integer *list, integer *lptr, 
	...
	ier)
{
    ...
    /* Local variables */
    integer i__, j, i1, i2, i3, n1, n2, n3, ka, kn, lp, kt, nm2, lp2, lpl, 
	    isv;
    logical arcs;
    integer lpln1;
...
/* Subroutine */ int sph_trmesh_(integer *n, doublereal *x, doublereal *y, 
	...
	*lnew, integer *near__, integer *next, doublereal *dist, integer *ier)
{
    ...
    integer i__, j, k;
    doublereal d1, d2, d3;
    integer i0, lp, nn, lpl;
    extern logical sph_left_(doublereal *, doublereal *, doublereal *, doublereal 
	    *, doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *);
    integer nexti;
    extern /* Subroutine */ int sph_addnod_(integer *, integer *, doublereal *, 
...
/* Store the first triangle in the linked list. */
    ...
    if (! sph_left_(&x[1], &y[1], &z__[1], &x[2], &y[2], &z__[2], &x[3], &y[3], &
	    z__[3])) {
...
/*   The first triangle is (3,2,1) = (2,1,3) = (1,3,2). */
    ...
    } else if (! sph_left_(&x[2], &y[2], &z__[2], &x[1], &y[1], &z__[1], &x[3], &
	    y[3], &z__[3])) {
...
/* Add the remaining nodes */
    ...
    i__1 = nn;
    for (k = 4; k <= i__1; ++k) {
	sph_addnod_(&near__[k], &k, &x[1], &y[1], &z__[1], &list[1], &lptr[1], &
		lend[1], lnew, ier);
	if (*ier != 0) {
	    return 0;
	}
