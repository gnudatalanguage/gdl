...
void HandleObjectsCallbacks() {
  for (g2gSubprocessListIter it = g2gListOfSubprocesses.begin(); it != g2gListOfSubprocesses.end(); ++it) {
	pid_t pid = (*it).first;
	int status = g2gListOfSubprocesses.at(pid);
	if (status == 2) {
	  g2gListOfSubprocesses.at(pid) = 0; //make completed
	  //perform generic callback
	  DObjGDL* o = g2gListOfObjects.at(pid);
	  ...
	  newEnv->SetNextPar(self); // pass 'self'
	  newEnv->SetNextPar(new DLongGDL(status)); // pass as local
	  if (status == 3) newEnv->SetNextPar(new DStringGDL("On error ???")); // pass as local
	  else if (status == 4) newEnv->SetNextPar(new DStringGDL("Aborted by User")); // pass as local
	  else newEnv->SetNextPar(new DStringGDL(""));
...
//  std::cout << SysVar::MsgPrefix() << "Signal from child " << pid << std::endl;
  g2gListOfSubprocesses.at(pid) = 2; //child just completed command
}
...
//   e->Throw("error sending command subprocess.");
  ...
  if (!nowait && g2gListOfSubprocesses.at(id[2]) == 1 ) {
...
std::map<pid_t,int> g2gListOfSubprocesses;
...
void gmem_setvar(EnvT* e) {
	...
	if (g2gListOfSubprocesses.at(subpid) == 1 ) e->Throw("Error: The object's associated IDL process is currently busy.");
...
BaseGDL* gmem_status(EnvT* e) {
	...
	if (p0->N_Elements() != 3) e->Throw("I need a triplet.");
 	DLongGDL *triplet = e->GetParAs<DLongGDL>(0);
       pid_t pid=(*triplet)[2];
		return new DIntGDL(g2gListOfSubprocesses.at(pid));
		static int ERROR=e->KeywordIx("ERROR");
		if (e->KeywordPresent(ERROR)) e->SetKW(ERROR,new DStringGDL(""));
}
...
//gmem_send must always trigger the callback, wait or nowait.
  ...
  void gmem_send(EnvT* e) {
	...
	pid_t pid = (*triplet)[2];
	if (g2gListOfSubprocesses.at(pid) == 1 ) e->Throw("Error: The object's associated IDL process is currently busy.");
  ...
  void gmem_abort(EnvT*e) {
	...
	if (p0->N_Elements()!=3) e->Throw("I need a triplet.");
	DLongGDL *triplet=e->GetParAs<DLongGDL>(0);
    g2gListOfSubprocesses.at((*triplet)[2]) = 4; //aborted
  ...
  BaseGDL* gmem_fork(EnvT* e) {
	...
	if (subprocess_pid == 0) {
	  ...
	  g2gListOfSubprocesses.insert(std::pair<pid_t, bool>(subprocess_pid, 0)); //idle
