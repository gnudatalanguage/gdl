 ...
 */
/* Intrinsic declarations for this to work "automatically" withing GDL */
#if defined(__SSE2__)
#define HAVE_SSE2
...
#define DSFMT_PST_INLINE
  ...
  * @return uint32 pseudorandom number
  */
 inline static uint32_t dsfmt_genrand_uint32(dsfmt_t *dsfmt) {
  ...
  /**
  * This function generates and returns a signed (but positive) 32-bit integer.
  * This is slower than SFMT, only for convenience usage.
  * dsfmt_init_gen_rand() or dsfmt_init_by_array() must be called
  * before this function.
  * @param dsfmt dsfmt internal state date
  * @return positive int32 pseudorandom number [0..2^31-1]
  */
 inline static int32_t dsfmt_genrand_int31(dsfmt_t *dsfmt) {
  int32_t r;
  uint64_t *psfmt64 = &dsfmt->status[0].u[0];
  do {
    if (dsfmt->idx >= DSFMT_N64) {
   ...
   r = psfmt64[dsfmt->idx++] & 0xffffffff;
  } while (r<0);
  return r;
 }
 /**
  * This function generates and returns unsigned 64-bit integer.
  * @param dsfmt dsfmt internal state date
  * @return uint64 pseudorandom number
  */
 inline static uint64_t dsfmt_genrand_uint64(dsfmt_t *dsfmt) {
  uint64_t *psfmt64 = &dsfmt->status[0].u[0];
