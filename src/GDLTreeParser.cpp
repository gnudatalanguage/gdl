...
void GDLTreeParser::lassign_expr(RefDNode _t) {
	...
	lassign_expr_AST=RefDNode(astFactory->make((new antlr::ASTArray(2))->add(antlr::RefAST(NULL))->add(antlr::RefAST(ex_AST))));
...
void GDLTreeParser::arrayindex_list_to_expression_list(RefDNode _t) {
	...
	for (;;) {
		...
		if ((_t->getType() == ARRAYIX)) {
			...
			if ( _cnt140>=1 ) { goto _loop140; } else {throw antlr::NoViableAltException(antlr::RefAST(_t));}
		}
