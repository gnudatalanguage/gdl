	...
	case DOT:
	{
		...
		for (;;) {
			...
			if ((_t->getType() == ARRAYEXPR || _t->getType() == EXPR || _t->getType() == IDENTIFIER)) {
				...
				if ( _cnt203>=1 ) { goto _loop203; } else {throw antlr::NoViableAltException(antlr::RefAST(_t));}
			}
...
void GDLTreeParser::arrayindex_list(RefDNode _t) {
	...
	for (;;) {
		...
		if ((_t->getType() == ARRAYIX)) {
			...
			if ( _cnt102>=1 ) { goto _loop102; } else {throw antlr::NoViableAltException(antlr::RefAST(_t));}
		}
	...
	case QUESTION:
	{
		...
		case ALL:
		{
			...
			case QUESTION:
			{
				...
				ax_AST->setType(ARRAYIX_ORANGE_S); // 5
				ax_AST->setText("s:*:s");
		...
		case QUESTION:
		{
			...
			case QUESTION:
			{
				...
				ax_AST->setType(ARRAYIX_RANGE_S); // 6
				ax_AST->setText("s:e:s");
