...
// AC 2020-04-28, then 2026-08-29
// see https://github.com/gnudatalanguage/gdl/issues/743
// and https://github.com/gnudatalanguage/gdl/issues/1400
// logic : if no lun numbers (list), all open files are listed
//         if lun numbers, only thosoe ones.
static void help_files(ostream& os, EnvT* e) {

  os << "Unit   Attributes                              Name\n";
  string out0="0      Read, Tty                             <stdin>\n";
  string out1="-1     Write, New, Tty                       <stdout>\n";
  string out2="-2     Write, New, Tty                       <stderr>\n";

  // two cases : need to scan all params, or scan all luns
  int nParam = e->NParam();
  //  cout << "var ?! "<<nParam  << endl;
  ...
  for (int p = 0; p < nParam; p++) {
    ...
    if (lun == -2) {os << out2;continue;}
    if (lun == -1) {os << out1;continue;}
    if (lun ==  0) {os << out0;continue;}
    
    GDLStream& file = fileUnits[lun - 1];
    if (!file.InUse())
      {os << std::left << std::setw(7) << lun << "Closed\n";}
    else
      {
	os << std::left<< std::setw(7) << lun;

	std::ostringstream milieu;
	if (file.IsReadable())
	  milieu << "Read";
	...
	os << std::left << std::setw(40) << milieu.str()<< file.Name()<<"\n";
  ...
  if (nParam ==0)
    {
      for (DLong lun = 1; lun <= static_cast<DLong>(fileUnits.size()); ++lun)
	{
	  GDLStream& file = fileUnits[lun - 1];
	  
	  if (!file.InUse() && !file.GetGetLunLock())
	    continue;
...
// following a message running Valgrind
...
static void help_Output(BaseGDL** outputKW, ostringstream& ostr, SizeT &nlines, bool doOutput=true)
{
  ...
  if(doOutput) {
    ...
    // commented by AC 26/08/29 because : out='' & HELP, out=out crashed !!
...
namespace lib {
  ...
  void help_pro(EnvT* e) {
    ...
    static int BREAKPOINTS = e->KeywordIx("BREAKPOINTS");
    if (e->KeywordPresent(BREAKPOINTS))
      { Message("Unsupported Keyword."); return;}
    
    static int MESSAGES = e->KeywordIx("MESSAGES");
    if (e->KeywordPresent(MESSAGES)) { Message("Unsupported Keyword."); return;}
    static int LAMBDA = e->KeywordIx("LAMBDA");
    if (e->KeywordPresent(LAMBDA)) { Message("Unsupported Keyword."); return;}
    ...
    // AC 26-08-29 : when PS, X/Y Offsets are updated and visible here when changed
