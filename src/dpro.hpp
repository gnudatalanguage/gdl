...
class DSub_compare: public std::function<bool(DSub*, DSub*)>
{
public:
  bool operator() (const DSub* f1, const DSub* f2) const { return f1->ObjectName() < f2->ObjectName();} //objectName as it adds the ":" so will work everywhere?
};
// Lib pro/fun ********************************************************
