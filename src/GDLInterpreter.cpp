...
BaseGDL*  GDLInterpreter::array_def(RefDNode _t) {
	...
	for (;;) {
		...
		if ((_tokenSet_1.member(_t->getType()))) {
			...
			if ( _cnt183>=1 ) { goto _loop183; } else {throw antlr::NoViableAltException(antlr::RefAST(_t));}
		}
...
BaseGDL*  GDLInterpreter::assign_expr(RefDNode _t) {
 ...
 BaseGDL*  GDLInterpreter::named_struct_def(RefDNode _t) {
	...
	for (;;) {
		...
		default:
		{
			if ( _cnt187>=1 ) { goto _loop187; } else {throw antlr::NoViableAltException(antlr::RefAST(_t));}
		}
		}
		_cnt187++;
	}
