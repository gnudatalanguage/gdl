...
#pragma omp for private(i)
      ...
      if (p1d->N_Elements() > p0->Rank() || (*p1d)[0] <= 0)
        e->Throw("Width must be a positive scalar or 1 (positive) element array in this context: " + e->GetParString(1));
      // No throw: a 2-elements [width,height] is allowed by our fast routine. However this is not standard IDL. Check and inform.
      if (p1d->N_Elements() == 2) { //other cases have been checked above, rest:
        if ((*p1d)[1] <= 0) e->Throw("Width must be >=1 for second array element in this context: " + e->GetParString(1));
      ...
      if (p0->Type() == GDL_BYTE && twoD && oddsize) {
        ...
        if (dbl) {
          ...
          if (twoD) {
            ...
            fastmedian::median_filter_2d(width, height, radius, radius, 0, (DDouble*) data->DataAddr(), (DDouble*) res->DataAddr(), iseven);
            return res;
          } else {
            BaseGDL* res = new DDoubleGDL(data->Dim(), BaseGDL::NOZERO);
            fastmedian::median_filter_1d(width, radius, 0, (DDouble*) data->DataAddr(), (DDouble*) res->DataAddr(), iseven);
            return res;
          }
        } else {
          ...
          if (twoD) {
            ...
            fastmedian::median_filter_2d(width, height, radius, radius, 0, (DFloat*) data->DataAddr(), (DFloat*) res->DataAddr(), iseven);
            return res;
          } else {
            BaseGDL* res = new DFloatGDL(data->Dim(), BaseGDL::NOZERO);
            fastmedian::median_filter_1d(width, radius, 0, (DFloat*) data->DataAddr(), (DFloat*) res->DataAddr(), iseven);
            return res;
          }
        }
      }
