...
if STRLOWCASE(!version.os_name) eq 'windows' then exit, status=0
  ...
  a->execute,"A+=1"
  b=a->getvar("A")
  if (total(b) ne 10) then ERRORS_ADD, nb_errors, 'BASIC'
end
...
; make shmap name unique, we do not want to get problems
  ...
  ; wait for 'c'
  s=c->status()
  while s lt 2 do begin
     wait,1
     s=c->status()
  endwhile
  ; check other's status is 2 as they must have returned earlier
  if a->status() ne 2 then  ERRORS_ADD, nb_errors, 'first process problem'
  if b->status() ne 2 then  ERRORS_ADD, nb_errors, 'second process problem'

  if result[0] ne 36 then ERRORS_ADD, nb_errors, 'total is bad'
  ...
  if result[0] ne 36 then ERRORS_ADD, nb_errors, 'subprocess abort problem'
