...
pro test_angles
  ...
  if !PI ne atan(1.) * 4 then message, '!PI != atan(1) * 4'
  if !DPI ne atan(1d) * 4 then message, '!DPI != atan(1) * 4'
  if !DTOR ne !PI / 180. then message, '!DTOR != !PI / 180.'
  if !RADEG ne 180. / !PI then message, '!RADEG != 180. / !PI'
        ...
        for d2 = 0, 3 do begin                  ; dimension of the second argument (0 - scalar)
          for ph = 0, 1 do begin                ; PHASE keyword presence
            ...
            if (types[t1] ne 6 && types[t1] ne 9 && types[t2] ne 6 && types[t2] ne 9) && $ 
              total(v lt -!PI or v gt !PI) gt 0      then message, 'FAILED (B.01)' + e
            ...
            ; B.04 for double input output should be double
                   ...
                   && s.type ne 5)                     then message, 'FAILED (B.04)' + e $
            ...
            if (d1 gt 0 && d2 gt 0 && ~ array_equal(s.dimensions, (size(d1 lt d2 ? $
              a1 : a2, /stru)).dimensions))            then message, 'FAILED (B.06)' + e
