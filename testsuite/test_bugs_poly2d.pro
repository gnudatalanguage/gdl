...
P=[0,-0.1,1,0] & Q=[0.9,1,0,0] & B = POLY_2D(A, P, Q, MISS=-1)
if ~ARRAY_EQUAL(B,good) then nb_errors++

P=[0.9,-0.1,1,0] & Q=[0.9,1,0,0] & B = POLY_2D(A, P, Q, MISS=-1)
if ~ARRAY_EQUAL(B,A) then nb_errors++
; small negative number is 0 if no MISS
P=[0,-1.2E-8,1,0] & Q=[0,1,0,0] & B = POLY_2D(A, P, Q)
if ~ARRAY_EQUAL(B,A) then nb_errors++
good=[  0b ,  1 ,  2 , 255 ,  4 ,  5 , 255 ,  7 ,  8]
P=[0,-1.2E-8,1,0] & Q=[0,1,0,0] & B = POLY_2D(A, P, Q, MISS=-1)
if ~ARRAY_EQUAL(B,good) then nb_errors++
; rotation
P=[2,0,-1,0] & Q=[2,-1,0,0] & B = POLY_2D(A, P, Q, MISS=-1)
good=[ 8b, 7, 6, 5, 4, 3, 2, 1, 0]
if ~ARRAY_EQUAL(B,good) then nb_errors++
; stretch x
a=bindgen(13,3) & P=[0,0,0.5,0] & Q=[0,1,0,0] & B = POLY_2D(A, P, Q, MISS=-1)
...
P=[-3,0,1,0] & Q=[0,1,0,0] & B = POLY_2D(A, P, Q,MISS=-1)
if ~ARRAY_EQUAL(B[*,1],good) then nb_errors++
; pixel
good=[255,    255,      3,    255,    255,      6,    255,    255,    255]
a=bindgen(3,3)
P=[-3,0,1,0] & Q=[0,1,0,0] & B = POLY_2D(A, P, Q,MISS=-1, pix=1)
if ~ARRAY_EQUAL(B,good) then nb_errors++
